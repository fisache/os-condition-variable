...
main(int argc, char* argv[]){
    ...
    //second sleep 1/2
    sleep(atoi(argv[2])/2);
    //if can't write, go to sleep
    while((scanFromFile("AW.txt")+scanFromFile("AR.txt")) > 0)
    {
        wait1(oktwid,lockid,pid);
    }
    //write
    printToFile("AW.txt",scanFromFile("AW.txt")+1,pid);
    //second sleep 2/2
    ...
    //complete to write
    printToFile("AW.txt",scanFromFile("AW.txt")-1,pid);
    //if sleep for other write, wake up
    if(scanFromFile("WW.txt") > 0)
    {
        signal1(oktwid,lockid,pid);
    //if sleep for other reads, all wake up
    }else if(scanFromFile("WR.txt") > 0){
        broadcast1(oktrid,lockid,pid);
    }
    //relase lock
    v(lockid);
}
